...
def main() -> None:
    ...
    s2 = sub.add_parser("step2", help="Valida, enriquece com cadastro e agrega por operadora/UF")
    s2.add_argument("--consolidated", default=str(data_dir("processed", "consolidado_despesas.csv")),
                    help="Caminho do consolidado gerado no step1")
    s2.add_argument("--cadastro-url",
    ...
    s2.add_argument("--invalid-cnpj-strategy", choices=["drop", "keep_mark"], default="drop",
                    help="Como tratar CNPJ inválido (recomendado: drop)")
    ...
    if args.cmd == "step2":
        ...
        out_csv = save_aggregated_csv(agg, filename="despesas_agregadas.csv")
        print(f"[OK] Consolidado enriquecido: {out.enriched_csv}")
        print(f"[OK] Agregado: {out_csv}")
        print(f"[OK] Invalid rows: {out.invalid_rows_csv}")
        print(f"[OK] Join sem match: {out.join_report_csv}")
